...
if(population=="host"){
  ...
  survey_complete_by_reach <- survey_complete_by_reach[order(
    as.numeric(substr(x = survey_complete_by_reach$camp_name, start = 6, nchar(survey_complete_by_reach$camp_name)))),]
}


# Create nice tables showing the above outputs:
kable_styling(kable_input = kable(x = survey_complete_by_reach, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                                  caption = "Total number of surveys per area (camp/union)."),
              position = "center")
...
check_duplicate_respondent_id <- hh_consent_daily %>% 
  ...
  select(c("reported_date","X_uuid","enumerator_id","respondent_id", "enumerator_name", "team", "instance_name")) %>%
...
if(population=="refugee"){
  ...
  FCN_missmatch <- left_join(x = hh_consent_daily[,c("X_uuid", "enumerator_id", "respondent_id", "reported_date", "enumerator_name", "team", "instance_name",
  ...
  FCN_missmatch = FCN_missmatch %>% select("X_uuid", "enumerator_id", "reported_date", "enumerator_name", "team","respondent_id",  "instance_name",
                                           "respondent_id", wrong_FCN, "hh_fcn", "FCN", wrong_camp, "camp_name", Camp)
...
if(population=="host"){
  ...
  admin_missmatch <- left_join(x = hh_consent_daily[,c("X_uuid", "enumerator_id", "reported_date", "enumerator_name", "team", "instance_name",
...
if(population=="host"){
  ...
  age_missmatch <- left_join(x = hh_consent_daily[,c("X_uuid", "enumerator_id", "reported_date", "enumerator_name", "team", "instance_name",
                                                     "respondent_id", "respondent_age", "resp_gender")],
                             y = sample_location,
                             by = c("respondent_id"="respondent_id"))
...
plot_ly(x = duration_df2$duration_minutes, type = "box", name = "Duration", height = 200, frame = TRUE, text = duration_df2$uuid)
```
...
survey_time <- left_join(x = duration_df2,
                         y = hh_consent_full %>% select(reported_date, X_uuid, enumerator_id, enumerator_name, team, "respondent_id", instance_name),
                         by = c("uuid"="X_uuid")) %>% 
  select(reported_date, uuid, enumerator_id, enumerator_name, team, "respondent_id", instance_name, duration_minutes)


# Calculate the survey times of the surveys in the monitoring period only:
survey_time_LT25 <- survey_time %>% filter(reported_date == day_to_run, duration_minutes <45) %>% rename("X_uuid"="uuid") # Don't think we need to filter by reported_date.

survey_time_LT25 <- survey_time_LT25[order(survey_time_LT25$duration_minutes, decreasing = FALSE),]

kable_styling(kable(x = survey_time_LT25, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE,
...
#### b) Duration statistics by enumerator – average time per survey per enumerator for monitoring period
...
overall_complete_by_enumerator <- data.frame(
  ...
  b = (hh_consent_daily %>% group_by(enumerator_id) %>% summarise("Monitoring_period" = n()))[2])
...
hh_pattern_dont_know <- hh_consent_daily %>% mutate(
  ...
  select("X_uuid", "enumerator_id", enumerator_name, team, respondent_id, instance_name, "hh_dont_know_rowsum")
...
dont_know_pattern_all <- hh_pattern_dont_know %>% left_join(indv_all_pattern_dont_know) %>% 
  ...
  select(X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, number_of_dont_know, everything())
...
individuals_difficulties = 
  left_join(x = individuals_difficulties,
            y = hh_consent_daily %>% 
              select(reported_date, X_uuid,enumerator_id, enumerator_name, team, respondent_id, instance_name, hh_size)) %>% 
  mutate(
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, hh_size, Indvs_with_Diffs, 
...
shelter_issues_reason_inconsistency <- hh_consent_daily %>% mutate(
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, shelter_issues_reason, leaks_during_rain, 
         lack_of_insulation, collapse_living_there, roof, window, structure, trap_heat, lack_insulation, limited_ventilation)
  

  # ---roof
  x = shelter_issues_reason_inconsistency[shelter_issues_reason_inconsistency[,"roof"]== "yes",] %>% 
    select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, 
           shelter_issues_reason, leaks_during_rain, lack_of_insulation, collapse_living_there, limited_ventilation)
  kable_styling(
    kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "roof"),
    position = "center")
  # ---window
    x = shelter_issues_reason_inconsistency[shelter_issues_reason_inconsistency[,"window"]== "yes",] %>% 
    select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, 
           shelter_issues_reason, leaks_during_rain, lack_of_insulation, collapse_living_there, limited_ventilation)
  kable_styling(
    kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "window"),
    position = "center")
  # ---structure
    x = shelter_issues_reason_inconsistency[shelter_issues_reason_inconsistency[,"structure"]== "yes",] %>% 
    select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, 
           shelter_issues_reason, leaks_during_rain, lack_of_insulation, collapse_living_there, limited_ventilation)
  kable_styling(
    kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "structure"),
    position = "center")
  # ---trap_heat
    x = shelter_issues_reason_inconsistency[shelter_issues_reason_inconsistency[,"trap_heat"]== "yes",] %>% 
    select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, 
           shelter_issues_reason, leaks_during_rain, lack_of_insulation, collapse_living_there, limited_ventilation)
  kable_styling(
    kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "trap_heat"),
    position = "center")
  # ---lack_insulation
    x = shelter_issues_reason_inconsistency[shelter_issues_reason_inconsistency[,"lack_insulation"]== "yes",] %>% 
    select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, 
           shelter_issues_reason, leaks_during_rain, lack_of_insulation, collapse_living_there, limited_ventilation)
  kable_styling(
    kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "lack_insulation"),
    position = "center")
...
improvement_reason_inconsistency <- hh_consent_daily %>% mutate(
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, improvement_reason, improvement_reason.did_not_receive_any_shelter_support, improvement_reason.sold_materials)
...
improvement_reason_inconsistency2 <- hh_consent_daily %>% left_join(able_males_18_59) %>% 
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, improvement_reason.no_able_bodied_person, able_males_18_59) 
...
improvement_reason_inconsistency2b <- hh_consent_daily %>% mutate(
  ...
  filter(conflict=="yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, improvement_reason.no_need_to_improve, leaks_during_rain, 
...
group_nfi_inconsistency <- hh_consent_daily %>% mutate(
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, hh_size, nfi_cols)
...
health_distance_inconsistensy <- hh_consent_daily %>% mutate(
  ...
  filter(conflict=="yes") %>% 
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, health_distance,
...
health_service_inconsistency<- hh_consent_daily %>% mutate(
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, health_barriers_accessed, health_barriers_not_accessed, 
...
to_far = health_service_inconsistency %>% filter(no_facility=="yes") %>%  
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, health_barriers_accessed, 
...
if(population == "refugee"){
  ...
  additional_girl_cols = c("distance_learning_girls_3", "distance_learning_girls_4_5", "distance_learning_girls_6_14",	
                           "distance_learning_girls_15_18", "distance_learning_girls_19_24")
  additional_boy_cols = c("distance_learning_boys_3", "distance_learning_boys_4_5", "distance_learning_boys_6_14",
  ...
  age_range = c(3,24)
    ...
    additional_girl_cols = c("distance_learning_girls_4_5", "distance_learning_girls_6_14", "distance_learning_girls_15_18")
    additional_boy_cols = c("distance_learning_boys_4_5", "distance_learning_boys_6_14", "distance_learning_boys_15_18")
...
relevant_cols = list(
  ...
  "lack_light" = c("living_space_issues", "solar_lamps"),
...
LI_dist_learning_barriers_girls <- hh_consent_daily %>% mutate(
  ...
  working_outside_home = case_when((distance_learning_barriers_girls.working_outside_home == 1 & child_income==0)~"yes", T~ "no"),
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, distance_learning_barriers_girls,
         ...
         additional_girl_cols)

  

# --working_outside_home
kable_styling(
  kable(x = LI_dist_learning_barriers_girls[LI_dist_learning_barriers_girls[,"working_outside_home"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name,
                 distance_learning_barriers_girls, relevant_cols[["working_outside_home"]], additional_girl_cols),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "working_outside_home"), position = "center")
  
  # --lack_content_older_children
kable_styling(
  kable(x = LI_dist_learning_barriers_girls[LI_dist_learning_barriers_girls[,"lack_content_older_children"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name,
                 distance_learning_barriers_girls, relevant_cols[["lack_content_older_children"]], additional_girl_cols),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "lack_content_older_children"), position = "center")

# --lack_content_younger_children
kable_styling(
  kable(x = LI_dist_learning_barriers_girls[LI_dist_learning_barriers_girls[,"lack_content_younger_children"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name,
                 distance_learning_barriers_girls, relevant_cols[["lack_content_younger_children"]], additional_girl_cols),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "lack_content_younger_children"), position = "center")

# --lack_light
kable_styling(
  kable(x = LI_dist_learning_barriers_girls[LI_dist_learning_barriers_girls[,"lack_light"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name,
                 distance_learning_barriers_girls, relevant_cols[["lack_light"]], additional_girl_cols),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "lack_light"), position = "center")

# --marriage_pregnancy
kable_styling(
  kable(x = LI_dist_learning_barriers_girls[LI_dist_learning_barriers_girls[,"marriage_pregnancy"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name,
                 distance_learning_barriers_girls, relevant_cols[["marriage_pregnancy"]], additional_girl_cols),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "marriage_pregnancy"), position = "center")
...
LI_dist_learning_barriers_boys <- hh_consent_daily %>% mutate(
  ...
  working_outside_home = case_when((distance_learning_barriers_boys.working_outside_home == 1 & child_income==0)~"yes", T~ "no"),
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, distance_learning_barriers_boys,
         working_outside_home, lack_content_older_children, lack_content_younger_children, lack_light, marriage_pregnancy, unique(unlist(relevant_cols)),
         additional_boy_cols)
...
# ---"working_outside_home"
kable_styling(
  kable(x = LI_dist_learning_barriers_boys[LI_dist_learning_barriers_boys[,"working_outside_home"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, distance_learning_barriers_boys, relevant_cols[["working_outside_home"]], additional_boy_cols),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "working_outside_home"), position = "center")
  
# ---"lack_content_older_children"
kable_styling(
  kable(x = LI_dist_learning_barriers_boys[LI_dist_learning_barriers_boys[,"lack_content_older_children"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, distance_learning_barriers_boys, relevant_cols[["lack_content_older_children"]], additional_boy_cols),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "lack_content_older_children"), position = "center")

# ---"lack_content_younger_children"
kable_styling(
  kable(x = LI_dist_learning_barriers_boys[LI_dist_learning_barriers_boys[,"lack_content_younger_children"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, distance_learning_barriers_boys, relevant_cols[["lack_content_younger_children"]], additional_boy_cols),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "lack_content_younger_children"), position = "center")

# ---"lack_light"
kable_styling(
  kable(x = LI_dist_learning_barriers_boys[LI_dist_learning_barriers_boys[,"lack_light"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, distance_learning_barriers_boys, relevant_cols[["lack_light"]], additional_boy_cols),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "lack_light"), position = "center")

# ---"marriage_pregnancy"
kable_styling(
  kable(x = LI_dist_learning_barriers_boys[LI_dist_learning_barriers_boys[,"marriage_pregnancy"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, distance_learning_barriers_boys, relevant_cols[["marriage_pregnancy"]], additional_boy_cols),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "marriage_pregnancy"), position = "center")
...
send_back_barriers_girls <- hh_consent_daily %>% mutate(
  ...
  older_girl_count = rowSums(hh_consent_daily[older_girls]),
  young_girl_count = rowSums(hh_consent_daily[young_girls]),
  working_outside_home =           case_when((send_back_barriers_girls.working_outside_home == 1 & child_income==0)~"yes", T~ "no"),
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, send_back_barriers_girls,
         working_outside_home, lack_content_older_children, lack_content_younger_children, marriage_pregnancy, unique(unlist(relevant_cols)), additional_girl_cols)



  kable_styling(
    kable(x = send_back_barriers_girls[send_back_barriers_girls[,"working_outside_home"]=="yes",] %>% 
            select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, send_back_barriers_girls, relevant_cols[["working_outside_home"]], additional_girl_cols), 
          longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "working_outside_home"), position = "center")

  
 kable_styling(
    kable(x = send_back_barriers_girls[send_back_barriers_girls[,"lack_content_older_children"]=="yes",] %>% 
            select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, send_back_barriers_girls, relevant_cols[["lack_content_older_children"]], additional_girl_cols), 
          longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "lack_content_older_children"), position = "center")
  

kable_styling(
    kable(x = send_back_barriers_girls[send_back_barriers_girls$lack[,"lack_content_younger_children"]=="yes",] %>% 
            select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, send_back_barriers_girls, relevant_cols[["lack_content_younger_children"]], additional_girl_cols), 
          longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "lack_content_younger_children"), position = "center")
  

 kable_styling(
    kable(x = send_back_barriers_girls[send_back_barriers_girls[,"marriage_pregnancy"]=="yes",] %>% 
            select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, send_back_barriers_girls, relevant_cols[["marriage_pregnancy"]], additional_girl_cols), 
          longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "marriage_pregnancy"), position = "center")
...
LI_send_back_barriers_boys <- hh_consent_daily %>% mutate(
  ...
  older_boy_count = rowSums(hh_consent_daily[older_boys]),
  young_boy_count = rowSums(hh_consent_daily[young_boys]),
  working_outside_home = case_when((send_back_barriers_boys.working_outside_home == 1 & child_income==0)~"yes", T~ "no"),
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, send_back_barriers_boys,
         working_outside_home, lack_content_older_children, lack_content_younger_children, marriage_pregnancy, unique(unlist(relevant_cols)), additional_boy_cols)


# --- "working_outside_home"
kable_styling(
  kable(x = LI_send_back_barriers_boys[LI_send_back_barriers_boys[,"working_outside_home"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, send_back_barriers_boys, 
                 relevant_cols[["working_outside_home"]], additional_boy_cols),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "working_outside_home"), position = "center")


# --- "lack_content_older_children"
kable_styling(
  kable(x = LI_send_back_barriers_boys[LI_send_back_barriers_boys[,"lack_content_older_children"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, send_back_barriers_boys, 
                 relevant_cols[["lack_content_older_children"]], additional_boy_cols),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "lack_content_older_children"), position = "center")

# --- "lack_content_younger_children"
kable_styling(
  kable(x = LI_send_back_barriers_boys[LI_send_back_barriers_boys[,"lack_content_younger_children"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, send_back_barriers_boys, 
                 relevant_cols[["lack_content_younger_children"]], additional_boy_cols),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "lack_content_younger_children"), position = "center")

# --- "marriage_pregnancy"
kable_styling(
  kable(x = LI_send_back_barriers_boys[LI_send_back_barriers_boys[,"marriage_pregnancy"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, send_back_barriers_boys, 
                 relevant_cols[["marriage_pregnancy"]], additional_boy_cols),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "marriage_pregnancy"), position = "center")
...
send_back_challenges_girls <- hh_consent_daily %>% mutate(
  ...
  older_girl_count = rowSums(hh_consent_daily[older_girls]),
  young_girl_count = rowSums(hh_consent_daily[young_girls]),
  working_outside_home  = case_when((send_back_challenges_girls.working_outside_home == 1 & child_income==0)~"yes", T~ "no"),
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, send_back_challenges_girls,
         working_outside_home, lack_content_older_children, lack_content_younger_children, marriage_pregnancy, unique(unlist(relevant_cols)), additional_girl_cols)


# --- "working_outside_home"
kable_styling(
  kable(x = send_back_challenges_girls[send_back_challenges_girls[,"working_outside_home"]=="yes",] %>% 
            select(reported_date, X_uuid, enumerator_id, enumerator_name, team, instance_name, respondent_id, send_back_challenges_girls, 
                   relevant_cols[["working_outside_home"]], additional_girl_cols), 
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "working_outside_home"), position = "center")

# --- "lack_content_older_children"
kable_styling(
  kable(x = send_back_challenges_girls[send_back_challenges_girls[,"lack_content_older_children"]=="yes",] %>% 
            select(reported_date, X_uuid, enumerator_id, enumerator_name, team, instance_name, respondent_id, send_back_challenges_girls, 
                   relevant_cols[["lack_content_older_children"]], additional_girl_cols), 
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "lack_content_older_children"), position = "center")

# --- "lack_content_younger_children"
kable_styling(
  kable(x = send_back_challenges_girls[send_back_challenges_girls[,"lack_content_younger_children"]=="yes",] %>% 
            select(reported_date, X_uuid, enumerator_id, enumerator_name, team, instance_name, respondent_id, send_back_challenges_girls, 
                   relevant_cols[["lack_content_younger_children"]], additional_girl_cols), 
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "lack_content_younger_children"), position = "center")

# --- "marriage_pregnancy"
kable_styling(
  kable(x = send_back_challenges_girls[send_back_challenges_girls[,"marriage_pregnancy"]=="yes",] %>% 
            select(reported_date, X_uuid, enumerator_id, enumerator_name, team, instance_name, respondent_id, send_back_challenges_girls, 
                   relevant_cols[["marriage_pregnancy"]], additional_girl_cols), 
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "marriage_pregnancy"), position = "center")
...
send_back_challenges_boys <- hh_consent_daily %>% mutate(
  ...
  older_boy_count = rowSums(hh_consent_daily[older_boys]),
  young_boy_count = rowSums(hh_consent_daily[young_boys]),
  working_outside_home = case_when((send_back_challenges_boys.working_outside_home == 1 & child_income==0)~"yes", T~ "no"),
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, send_back_challenges_boys,
         working_outside_home, lack_content_older_children, lack_content_younger_children, marriage_pregnancy, unique(unlist(relevant_cols)), additional_boy_cols)
...
# --- "working_outside_home"
kable_styling(
  kable(x = send_back_challenges_boys[send_back_challenges_boys[,"working_outside_home"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, send_back_challenges_boys, 
                 relevant_cols[["working_outside_home"]], additional_boy_cols), 
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "working_outside_home"), position = "center")
...
# ---"lack_content_older_children"
kable_styling(
  kable(x = send_back_challenges_boys[send_back_challenges_boys[,"lack_content_older_children"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, send_back_challenges_boys, 
                 relevant_cols[["lack_content_older_children"]], additional_boy_cols), 
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "lack_content_older_children"), position = "center")
...
# --- "lack_content_younger_children"
kable_styling(
  kable(x = send_back_challenges_boys[send_back_challenges_boys[,"lack_content_younger_children"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, send_back_challenges_boys, 
                 relevant_cols[["lack_content_younger_children"]], additional_boy_cols), 
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "lack_content_younger_children"), position = "center")
...
# --- "marriage_pregnancy"
kable_styling(
  kable(x = send_back_challenges_boys[send_back_challenges_boys[,"marriage_pregnancy"]=="yes",] %>% 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, send_back_challenges_boys, 
                 relevant_cols[["marriage_pregnancy"]], additional_boy_cols), 
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "marriage_pregnancy"), position = "center")
...
nutrition_barriers_not_accessed_inconsistency <- hh_consent_daily %>% mutate(
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name,
         ...
         child_taken_no_support, child_not_taken_after_referral, nutrition_barriers_not_accessed)
...
kable_styling(kable(x = nutrition_barriers_not_accessed_inconsistency %>% 
                      ...
                      select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name,
                             ...
                             nutrition_barriers_not_accessed, child_taken_no_support_HH_count),
                    longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE,
...
kable_styling(kable(x = nutrition_barriers_not_accessed_inconsistency %>% 
                      ...
                      select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name,
                             # child_taken_no_support, child_not_taken_after_referral,
                             # nutrition_barriers_not_accessed_cols, nutrition_barriers_not_accessed.already_referred,
                             nutrition_barriers_not_accessed, child_not_taken_after_referral_HH_count),
                    longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE,
...
if(population=="refugee"){
  ...
  plw = hh_consent_daily[index, c("reported_date", "X_uuid", "enumerator_id", "respondent_id", "plw_admitted", "plw_sfs")]
...
if(population=="refugee"){
  ...
  food_barriers <- hh_consent_daily %>% mutate(
    Flag = case_when(((food_barriers.no_assistance == 1) |
      ...
      select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, Flag, feedback)
...
relevant_cols = list(
  ...
  "child_working_long_hours" = c("child_working_long_hours", "child_income"), 
...
group_livelihood_coping_inconsistency <- hh_consent_daily %>% mutate(
  ...
  indv_with_income = indv_with_income, child_income = child_income, adult_income = adult_income,
  ...
  child_working_long_hours = case_when((child_working_long_hours == "yes" & child_income ==0)~"yes", TRUE~"no"),
  ...
  select(c(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name,
           main_income_source, unique(unlist(relevant_cols))))


# --- "selling_food_rations"
kable_styling(
  kable(x = group_livelihood_coping_inconsistency[group_livelihood_coping_inconsistency[,"selling_food_rations"]=="yes",] %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name,
                 relevant_cols[["selling_food_rations"]]),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "selling_food_rations"), position = "center")

# --- "child_working_long_hours"
kable_styling(
  kable(x = group_livelihood_coping_inconsistency[group_livelihood_coping_inconsistency[,"child_working_long_hours"]=="yes",] %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name,
                 relevant_cols[["child_working_long_hours"]]),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "child_working_long_hours"), position = "center")

# --- "child_marriage"
kable_styling(
  kable(x = group_livelihood_coping_inconsistency[group_livelihood_coping_inconsistency[,"child_marriage"]=="yes",] %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name,
                 relevant_cols[["child_marriage"]]),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "child_marriage"), position = "center")

# --- "risk_jobs"
kable_styling(
  kable(x = group_livelihood_coping_inconsistency[group_livelihood_coping_inconsistency[,"risk_jobs"]=="yes",] %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name,
                 relevant_cols[["risk_jobs"]]),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "risk_jobs"), position = "center")

# --- "adults_working_long_hours"
kable_styling(
  kable(x = group_livelihood_coping_inconsistency[group_livelihood_coping_inconsistency[,"adults_working_long_hours"]=="yes",] %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name,
                 relevant_cols[["adults_working_long_hours"]]),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "adults_working_long_hours"), position = "center")
...
coping_reason_inconsistency_2 <- hh_consent_daily %>% mutate(
  ...
  clothes = case_when(((coping_reason.clothing ==1 |coping_reason.hh_items==1) & freq_nfi_expenditure==0)~"yes", TRUE~ "no"),
...
# cols = c("reported_date", "X_uuid", "enumerator_id", "enumerator_name", "team", "instance_name", "coping_reason", "food_expenditure", 
...
caps = c("== “food” & food_expenditure == 0",
         "== “healthcare” & (paid_healthcare != “yes” | health_expenditure == 0)",
         "== “education” & no children in distance learning  (or returned, if relevant)",
         "== “education” & education_expenditure == 0",
         "== (“clothing” | “hh_items”) & infreq_nfi_expenditure == 0",
         "== “shelter” & (improvement == “no_improvement” | improvement == “dont_know” | improvement == “other” | shelter_expenditure == 0)",
         "== “cooking_fuel” & fuel != “buying_lpg_refills” & fuel != “firewood_purchased” ",
         "== “cooking_fuel” & fuel_expenditure == 0",
         "== “hygiene_items” & freq_nfi_expenditure == 0",
         "== “water” & (water_coping != “spend_money” & water_coping != “buy_water”)",
         "== “water” & water_expenditure == 0")


relevant_columns = list(
  "food" = c("food_expenditure"),
  "healthcare" = c("paid_healthcare", "health_expenditure"), 
  "edu" = c(dist_learning_cols), 
  "edu2" = c("education_expenditure"), 
  "clothes" = c("coping_reason.hh_items", "freq_nfi_expenditure"), 
  # rent added below
  ...
  "water1" = c("water_coping.spend_money"),
  "water2" = c("water_expenditure")
) 
...
if(population=="refugee"){
  ...
  caps = c(caps, "== “rent” & (shelter_paid == “no_need” | shelter_paid == “dont_know” | shelter_paid == ”other” | rent_expenditure == 0)")
}
...
# ---"food"
concol = conflict_cols[1]
kable_styling(
  kable(x = coping_reason_inconsistency_2 %>% filter(coping_reason_inconsistency_2[concol] == "yes") %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, coping_reason, relevant_columns[[concol]]),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = caps[1]), position = "center")

# ---
concol = conflict_cols[2]
kable_styling(
  kable(x = coping_reason_inconsistency_2 %>% filter(coping_reason_inconsistency_2[concol] == "yes") %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, coping_reason, relevant_columns[[concol]]),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = caps[2]), position = "center")

# ---
concol = conflict_cols[3]
kable_styling(
  kable(x = coping_reason_inconsistency_2 %>% filter(coping_reason_inconsistency_2[concol] == "yes") %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, coping_reason, relevant_columns[[concol]]),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = caps[3]), position = "center")

# ---
concol = conflict_cols[4]
kable_styling(
  kable(x = coping_reason_inconsistency_2 %>% filter(coping_reason_inconsistency_2[concol] == "yes") %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, coping_reason, relevant_columns[[concol]]),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = caps[4]), position = "center")

# ---
concol = conflict_cols[5]
kable_styling(
  kable(x = coping_reason_inconsistency_2 %>% filter(coping_reason_inconsistency_2[concol] == "yes") %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, coping_reason, relevant_columns[[concol]]),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = caps[5]), position = "center")

# ---
concol = conflict_cols[6]
kable_styling(
  kable(x = coping_reason_inconsistency_2 %>% filter(coping_reason_inconsistency_2[concol] == "yes") %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, coping_reason, relevant_columns[[concol]]),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = caps[6]), position = "center")

# ---
concol = conflict_cols[7]
kable_styling(
  kable(x = coping_reason_inconsistency_2 %>% filter(coping_reason_inconsistency_2[concol] == "yes") %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, coping_reason, relevant_columns[[concol]]),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = caps[7]), position = "center")

# ---
concol = conflict_cols[8]
kable_styling(
  kable(x = coping_reason_inconsistency_2 %>% filter(coping_reason_inconsistency_2[concol] == "yes") %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, coping_reason, relevant_columns[[concol]]),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = caps[8]), position = "center")

# ---
concol = conflict_cols[9]
kable_styling(
  kable(x = coping_reason_inconsistency_2 %>% filter(coping_reason_inconsistency_2[concol] == "yes") %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, coping_reason, relevant_columns[[concol]]),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = caps[9]), position = "center")

# ---
concol = conflict_cols[10]
kable_styling(
  kable(x = coping_reason_inconsistency_2 %>% filter(coping_reason_inconsistency_2[concol] == "yes") %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, coping_reason, relevant_columns[[concol]]),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = caps[10]), position = "center")

# ---
concol = conflict_cols[11]
kable_styling(
  kable(x = coping_reason_inconsistency_2 %>% filter(coping_reason_inconsistency_2[concol] == "yes") %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, coping_reason, relevant_columns[[concol]]),
        longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = caps[11]), position = "center")
...
amounts_data_day <- hh_consent_daily[, c("reported_date", "X_uuid", "enumerator_id","enumerator_name", "team", "respondent_id", "instance_name", colnames(amounts))]
...
amounts_data_day = amounts_data_day %>% select(c("reported_date", "X_uuid", "enumerator_id", "enumerator_name", "team", respondent_id, "instance_name", 
           colnames(amounts), gsub(pattern = "_amount", replacement = "", x = colnames(amounts))))

  
# --- 
concol = gsub(pattern = "_amount", replacement = "", x = colnames(amounts))[1]
x = amounts_data_day %>% filter(amounts_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts)[1], concol)
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, "   |  Lower limit is ", round(amounts[3,paste0(concol, "_amount")], 0),
                                           ". Upper limit is ", round(amounts[4,paste0(concol, "_amount")], 0))),  position = "center")

# --- 
concol = gsub(pattern = "_amount", replacement = "", x = colnames(amounts))[2]
x = amounts_data_day %>% filter(amounts_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts)[2], concol)
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, "   |  Lower limit is ", round(amounts[3,paste0(concol, "_amount")], 0),
                                           ". Upper limit is ", round(amounts[4,paste0(concol, "_amount")], 0))),  position = "center")

# --- 
concol = gsub(pattern = "_amount", replacement = "", x = colnames(amounts))[3]
x = amounts_data_day %>% filter(amounts_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts)[3], concol)
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, "   |  Lower limit is ", round(amounts[3,paste0(concol, "_amount")], 0),
                                           ". Upper limit is ", round(amounts[4,paste0(concol, "_amount")], 0))),  position = "center")

# --- 
concol = gsub(pattern = "_amount", replacement = "", x = colnames(amounts))[4]
x = amounts_data_day %>% filter(amounts_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts)[4], concol)
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, "   |  Lower limit is ", round(amounts[3,paste0(concol, "_amount")], 0),
                                           ". Upper limit is ", round(amounts[4,paste0(concol, "_amount")], 0))),  position = "center")

# --- 
concol = gsub(pattern = "_amount", replacement = "", x = colnames(amounts))[5]
x = amounts_data_day %>% filter(amounts_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts)[5], concol)
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, "   |  Lower limit is ", round(amounts[3,paste0(concol, "_amount")], 0),
                                           ". Upper limit is ", round(amounts[4,paste0(concol, "_amount")], 0))),  position = "center")

# --- 
concol = gsub(pattern = "_amount", replacement = "", x = colnames(amounts))[6]
x = amounts_data_day %>% filter(amounts_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts)[6], concol)
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, "   |  Lower limit is ", round(amounts[3,paste0(concol, "_amount")], 0),
                                           ". Upper limit is ", round(amounts[4,paste0(concol, "_amount")], 0))),  position = "center")

# --- 
concol = gsub(pattern = "_amount", replacement = "", x = colnames(amounts))[7]
x = amounts_data_day %>% filter(amounts_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts)[7], concol)
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, "   |  Lower limit is ", round(amounts[3,paste0(concol, "_amount")], 0),
                                           ". Upper limit is ", round(amounts[4,paste0(concol, "_amount")], 0))),  position = "center")

# --- 
concol = gsub(pattern = "_amount", replacement = "", x = colnames(amounts))[8]
x = amounts_data_day %>% filter(amounts_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts)[8], concol)
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, "   |  Lower limit is ", round(amounts[3,paste0(concol, "_amount")], 0),
                                           ". Upper limit is ", round(amounts[4,paste0(concol, "_amount")], 0))),  position = "center")

# --- 
concol = gsub(pattern = "_amount", replacement = "", x = colnames(amounts))[9]
x = amounts_data_day %>% filter(amounts_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts)[9], concol)
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, "   |  Lower limit is ", round(amounts[3,paste0(concol, "_amount")], 0),
                                           ". Upper limit is ", round(amounts[4,paste0(concol, "_amount")], 0))),  position = "center")

# --- 
concol = gsub(pattern = "_amount", replacement = "", x = colnames(amounts))[10]
x = amounts_data_day %>% filter(amounts_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts)[10], concol)
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, "   |  Lower limit is ", round(amounts[3,paste0(concol, "_amount")], 0),
                                           ". Upper limit is ", round(amounts[4,paste0(concol, "_amount")], 0))),  position = "center")

# --- 
concol = gsub(pattern = "_amount", replacement = "", x = colnames(amounts))[11]
x = amounts_data_day %>% filter(amounts_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts)[11], concol)
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, "   |  Lower limit is ", round(amounts[3,paste0(concol, "_amount")], 0),
                                           ". Upper limit is ", round(amounts[4,paste0(concol, "_amount")], 0))),  position = "center")
...
# --- 
if(population=="host"){
concol = gsub(pattern = "_amount", replacement = "", x = colnames(amounts))[12]
x = amounts_data_day %>% filter(amounts_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts)[12], concol)
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, "   |  Lower limit is ", round(amounts[3,paste0(concol, "_amount")], 0),
                                           ". Upper limit is ", round(amounts[4,paste0(concol, "_amount")], 0))),  position = "center")
}
...
Checks of the following PER CAPITA: food_expenditure, rent_expenditure , water_expenditure, freq_nfi_expenditure, fuel_expenditure, transportation_expenditure
```{r eval=TRUE, echo=FALSE, results='asis', warning=FALSE, messages=FALSE}

amounts2 = c("food_expenditure", "rent_expenditure" , "water_expenditure", "freq_nfi_expenditure", "fuel_expenditure", "transportation_expenditure")
...
# Calculate the daily amount totals:
hh_consent_daily$total_expenditure <- rowSums(hh_consent_daily[, colnames(amounts)[1:(ncol(amounts)-1)]], na.rm = TRUE)

# Create a subset of the data to use for calculations
amounts2_data_day <- hh_consent_daily[, colnames(amounts2)]

# Convert to per Capita:
amounts2_data_day = amounts2_data_day[colnames(amounts2)] / hh_consent_daily$hh_size
...
amounts2_data_day = cbind(hh_consent_daily[, c("reported_date", "X_uuid", "enumerator_id","enumerator_name", "team", "respondent_id", "instance_name")],
                          amounts2_data_day)
...
# --- 
concol = gsub(pattern = "_expenditure", replacement = "", x = colnames(amounts2))[1]
x = amounts2_data_day[amounts2_data_day[concol] == "yes",] %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts2)[1], concol)

kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                    caption = paste0(concol, "   |  Lower limit is ", round(amounts2[3,paste0(concol, "_expenditure")], 0),
                                     ". Upper limit is ", round(amounts2[4,paste0(concol, "_expenditure")], 0))),  position = "center")

# --- 
concol = gsub(pattern = "_expenditure", replacement = "", x = colnames(amounts2))[2]
x = amounts2_data_day[amounts2_data_day[concol] == "yes",] %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts2)[2], concol)
...
# --- 
concol = gsub(pattern = "_expenditure", replacement = "", x = colnames(amounts2))[3]
x = amounts2_data_day[amounts2_data_day[concol] == "yes",] %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts2)[3], concol)
...
# --- 
concol = gsub(pattern = "_expenditure", replacement = "", x = colnames(amounts2))[4]
x = amounts2_data_day[amounts2_data_day[concol] == "yes",] %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts2)[4], concol)
...
# --- 
concol = gsub(pattern = "_expenditure", replacement = "", x = colnames(amounts2))[5]
x = amounts2_data_day[amounts2_data_day[concol] == "yes",] %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts2)[5], concol)
...
# --- 
concol = gsub(pattern = "_expenditure", replacement = "", x = colnames(amounts2))[6]
x = amounts2_data_day[amounts2_data_day[concol] == "yes",] %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts2)[6], concol)
...
# --- 
concol = gsub(pattern = "_expenditure", replacement = "", x = colnames(amounts2))[7]
x = amounts2_data_day[amounts2_data_day[concol] == "yes",] %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts2)[7], concol)
...
if(population=="refugee"){
  rent_expenditure_issue = hh_consent_daily %>% mutate(
    ...
    filter(conflict=="yes") %>% 
    select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, shelter_paid, rent_expenditure)
...
fuel_expenditure_inconsistency <- hh_consent_daily %>% mutate(
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, instance_name, respondent_id, fuel_expenditure, cooking_fuel_cols, fuel_expend_inconsistency)
...
Expenditure: Should flag unusually high/low values for each and the sum. [PER CAPITA]
```{r eval=TRUE, echo=FALSE, results='asis', warning=FALSE, messages=FALSE}
...
expenditures_data_day <- hh_consent_daily[, c("reported_date", "X_uuid", "enumerator_id", "enumerator_name", "team", "respondent_id", "instance_name",  colnames(expenditures))]
...
expenditures_data_day <- expenditures_data_day %>% mutate(
  ...
  exp_total  = case_when((expenditure_total < expenditures[3, "expenditure_total"] | 
                            expenditure_total > expenditures[4, "expenditure_total"])~"yes", TRUE~"no"))
...
x = expenditures_data_day %>% filter(expenditures_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, relevant_cols[1])
  
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, ":  Upper limit is ", round(expenditures[4,relevant_cols[1]], 0))), 
                    position = "center")

# ---
concol = cols[2]
x = expenditures_data_day %>% filter(expenditures_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, relevant_cols[2])
  
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, ":  Upper limit is ", round(expenditures[4,relevant_cols[2]], 0))), 
                    position = "center")

# ---
concol = cols[3]
x = expenditures_data_day %>% filter(expenditures_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, relevant_cols[3])
  
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, ":  Upper limit is ", round(expenditures[4,relevant_cols[3]], 0))), 
                    position = "center")

# ---
concol = cols[4]
x = expenditures_data_day %>% filter(expenditures_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, relevant_cols[4])
  
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, ":  Upper limit is ", round(expenditures[4,relevant_cols[4]], 0))), 
                    position = "center")

# ---
concol = cols[5]
x = expenditures_data_day %>% filter(expenditures_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, relevant_cols[5])
  
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, ":  Upper limit is ", round(expenditures[4,relevant_cols[5]], 0))), 
                    position = "center")

# ---
concol = cols[6]
x = expenditures_data_day %>% filter(expenditures_data_day[concol] == "yes") %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, relevant_cols[6])
  
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, 
                          caption = paste0(concol, ":  Upper limit is ", round(expenditures[4,relevant_cols[6]], 0))), 
                    position = "center")
...
health_expenditure_inconsistency<- hh_consent_daily %>% mutate(
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, paid_healthcare, health_expenditure)
...
shelter_expenditure_inconsistency <- hh_consent_daily %>% mutate(
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, shelter_expenditure, shelter_materials_source)
...
#   select(reported_date,X_uuid,enumerator_id, enumerator_name, team, instance_name,sanitation_coping,sanitation_barriers_male,sanitation_barriers_female,sanitation_coping_inconsistency) %>% 
...
group_water_quantity = hh_consent_daily %>% mutate(
  ...
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, sanitation_coping, enough_water_bathing_facility, enough_water_bathing_shelter)
...
kable_styling(kable(x = group_water_quantity, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE,
...
hh_other_cols = hh_other_cols[!hh_other_cols %in% c("water_coping.less_preferred_other", "water_coping.less_use_other", "enough_water_other")]

# Subset the data to just the columns with other data:
hh_other <- hh_consent_daily %>%
  filter_at(vars(hh_other_cols), any_vars(!is.na(.))) %>%
  select(reported_date,X_uuid,enumerator_id, enumerator_name, team, respondent_id, instance_name, hh_other_cols)
...
if(nrow(hh_other)>0){
  ...
  hh_other_pi_long <- hh_other %>% 
    pivot_longer(-c("reported_date","X_uuid","enumerator_id", "enumerator_name", "respondent_id", "team", "instance_name"),
...
3.f.	Number of issues flagged by enumerator: [needs an update]
```{r eval=TRUE, echo=FALSE, results='asis', warning=FALSE, messages=FALSE}
...
final_cleaning_log <- final_cleaning_log %>% mutate(new_value =NULL,change_type=NULL) %>%
  select(reported_date, X_uuid, enumerator_id, enumerator_name, team, instance_name, survey_loop, questions, old_value, everything())
