...
data_path     <- paste0("../02 Data Collection and Planning/Quant/4_Data_collection/", population, "/Day4/")
audit_zip_dir <- paste0("../02 Data Collection and Planning/Quant/4_Data_collection/", population, "/Day4/")
...
# CAMP TUESDAY
# data_file_name  <- "Camp_JMSNA_2021_-_latest_version_-_False_-_2021-07-13-13-15-57.xlsx"
# audit_name      <- "a5C6hZmgWjP2NcTwt5GAb8_2021_07_13_13_17_34"
...
duplicate_id = hh_consent_full$respondent_id[duplicated(hh_consent_full$respondent_id)]
...
# Restructure if you would like to add it to the cleaning log
...
survey_time_LT45 <- survey_time %>% filter(reported_date == day_to_run, duration_minutes <45) %>% rename("X_uuid"="uuid") # Don't think we need to filter by reported_date.
...
#### b) Duration statistics by enumerator – average time per survey for whole data collection period
...
shelter_issues_reason_inconsistency <- hh_consent_daily %>% mutate(
  ...
  colapse = case_when((hh_consent_daily$collapse_living_there =="yes" | hh_consent_daily$collapse_with_other_hh =="yes" | 
                         hh_consent_daily$with_other_hh =="yes" | hh_consent_daily$collapse_in_open =="yes")~"yes",T~ "no"))
...
if(population == "refugee"){
  ...
  shelter_issues_reason_inconsistency = shelter_issues_reason_inconsistency %>% 
    select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, shelter_issues_reason, leaks_during_rain, 
         ...
         colapse, collapse_with_other_hh, with_other_hh, collapse_in_open)
...
x = shelter_issues_reason_inconsistency[shelter_issues_reason_inconsistency$colapse== "yes",] %>% 
    select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, 
           shelter_issues_reason, leaks_during_rain, lack_of_insulation, collapse_living_there, limited_ventilation,
           colapse, collapse_with_other_hh, with_other_hh, collapse_in_open)
kable_styling(kable(x = x, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE, caption = "Colapse"), "striped")
# }
...
for(i in gender){
  ...
  if(length(which(flags==TRUE))>0){
    ...
    flag_data = hh_consent_daily[, c("reported_date", "X_uuid", "enumerator_id", "enumerator_name","team", "respondent_id", "instance_name", 
                                              not_enroled_cols, child_count_col, child_enrolled_col)]
...
kable_styling(kable(x = nutrition_barriers_not_accessed_inconsistency %>% 
                      ...
                      select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, repeat_instance_no_support,
...
kable_styling(kable(x = nutrition_barriers_not_accessed_inconsistency %>% 
                      ...
                      select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, repeat_instance_not_taken,
                             # child_taken_no_support, child_not_taken_after_referral,
                             # nutrition_barriers_not_accessed_cols, nutrition_barriers_not_accessed.already_referred,
                             child_taken_no_support_HH_count, child_not_taken_after_referral_HH_count, nutrition_barriers_not_accessed),
                    longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE,
...
if(population=="refugee"){
  ...
  food_barriers <- hh_consent_daily %>% mutate(
    Flag = case_when(((food_barriers.no_assistance == 1) |
      ...
      select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, food_barriers, feedback, food_barriers.no_assistance)
...
kable_styling(kable(x = hh_consent_daily[c(hh_consent_daily$begging=="exhaused" | hh_consent_daily$begging=="not_available"), 
                     c("reported_date", "X_uuid", "enumerator_id", "enumerator_name", "team", "respondent_id", "instance_name", "begging")],
    longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE,
...
kable_styling(kable(x = hh_consent_daily[
  ...
  c("reported_date", "X_uuid", "enumerator_id", "enumerator_name", "team", "respondent_id", "instance_name",
...
# For all strategies “not_available” is selected:
  ...
  # Print table
  kable_styling(kable(x = coping_strategies, longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE,
...
for(i in names(relevant_columns)){
  ...
  kable(x = coping_reason_inconsistency_2 %>% filter(coping_reason_inconsistency_2[i] == "yes") %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, coping_reason, relevant_columns[[i]]),
...
kable_styling(kable(x = hh_consent_daily[hh_consent_daily$main_income_source.humanitarian_assistance==0,
                                           c("reported_date", "X_uuid", "enumerator_id", "enumerator_name", "team", "respondent_id",
                                             "instance_name","main_income_source.humanitarian_assistance")],
      longtable = T, booktabs = T, linesep = "", align = "c", row.names = FALSE,
...
amounts3_data_day = cbind(hh_consent_daily[, c("reported_date", "X_uuid", "enumerator_id","enumerator_name", "team", "respondent_id", "instance_name")],
                          amounts3_data_day)
...
for(i in 1:ncol(amounts3)){
  ...
  # Select per Capita data:
  x = amounts3_data_day %>%
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, colnames(amounts3)[i], concol)
...
# print the table
food_value = cbind(hh_consent_daily[c("reported_date", "X_uuid", "enumerator_id", "enumerator_name", "team", "respondent_id", "instance_name")], food_value)
...
hh_consent_daily$total_expenditure <- rowSums(hh_consent_daily[, colnames(expenditures)[1:5]], na.rm = TRUE)
...
expenditures_data_day <- expenditures_data_day %>% mutate(
  ...
  livelihood_exp = case_when((education_expenditure < expenditures[3, "livelihood_expenditure"] |
                               education_expenditure > expenditures[4, "livelihood_expenditure"])~"yes", TRUE~"no"),
...
for(i in 1:length(cols)){
  ...
  x = expenditures_data_day %>% # %>% filter(expenditures_data_day[concol] == "yes") 
          select(reported_date, X_uuid, enumerator_id, enumerator_name, team, respondent_id, instance_name, relevant_cols[i], concol)
...
dont_knows = hh_consent_daily[c("reported_date","X_uuid","enumerator_id", "enumerator_name", "respondent_id", "team", "instance_name", dont_knows)]
...
# Print out the remaining dataset:
dont_knows = dont_knows[dont_knows$flag>0, c("reported_date","X_uuid","enumerator_id", "enumerator_name", "respondent_id", "team", "instance_name",c)]
